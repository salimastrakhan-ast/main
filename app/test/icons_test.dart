...
import 'package:tito/ui/icons.dart';
...
void main() {
  group('Набор иконок', () {
    test('все из одного семейства', () {
      ...
      for (final icon in TitoIcons.all) {
    ...
    test('ни одна иконка не повторяется', () {
      final codes = TitoIcons.all.map((i) => i.codePoint).toList();
      expect(codes.toSet().length, codes.length,
          reason: 'две роли указывают на одну картинку');
    });

    test('набор не пуст и покрывает все роли', () {
      // Число сверяется вручную не от лени: оно ловит забытую строку в
      // списке `all`, а без неё иконка не попадает ни в этот тест, ни в
      // проверку на повторы — и роль с чужой картинкой проходит незаметно.
      expect(TitoIcons.all, hasLength(57));
    });
  });
}
