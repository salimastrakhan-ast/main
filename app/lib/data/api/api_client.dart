...
class ApiClient {
  ...
  /// Адреса серверов для звонка.
  ///
  /// STUN сообщает клиенту, как он выглядит снаружи; TURN пересылает звук
  /// через себя, когда прямое соединение не встаёт. Список берётся на
  /// каждый звонок: пароль к TURN живёт часы, а не вечно.
  Future<List<dynamic>> iceServers() async {
    final json = await _get('/v1/calls/ice');
    return json['ice_servers'] as List<dynamic>? ?? const [];
  }
